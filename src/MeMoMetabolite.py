...
from src.handle_metabolites_prefix_suffix import (
    handle_metabolites_prefix_suffix,
    validateInchi,
)
...
class MeMoMetabolite:
    ...
    def set_id(self, new_id: str) -> None:
        ...
        if self._id is not None:
            warnings.warn(
                "changed metbolite _id from {old} to {new}".format(
                    old=self._id, new=new_id
                )
            )
    ...
    def set_model_id(self, new_model_id: str) -> None:
        ...
        if self._model_id is not None:
            warnings.warn(
                "changed metbolite _model_id from {old} to {new}".format(
                    old=self._model_id, new=new_model_id
                )
            )
        self._model_id = new_model_id

    def set_names(
        self, new_names: list[str], source: str | list[str] | None = None
    ) -> None:
        ...
        if len(source) != len(new_names):
            raise ValueError(
                "List of new names have a different length than the one for the sources. Consider providing a corresponding soure list to the new names via the source parameter."
            )

        if self.names:
            warnings.warn(
                "changed metbolite names from {old} to {new}".format(
                    old=str(self.names), new=str(new_names)
                )
            )
    ...
    def add_names(self, new_names: list[str], source: str) -> int:
        ...
        self.names_source.extend(
            [source] * (len(self.names) - len(old_names))
        )  # extend source list
        # we want the names to be sorted lexographically, thus we have to sort the sources accordingly, so that the index of the name corresponds to the index of the source. This is done with this rather complicated line below (stackoverflow for the rescue)
        new_names, new_sources = zip(
            *sorted(zip(self.names, self.names_source))
        )  # sort both list for self.names
        self.names = list(new_names)
        ...
        if self.orig_ids != []:
            warnings.warn(
                "changed metbolite orig_ids from {old} to {new}".format(
                    old=str(self.orig_ids), new=str(new_orig_ids)
                )
            )
    ...
    def set_formula(self, new_formula: str) -> None:
        ...
        if self._formula is not None:
            warnings.warn(
                "changed metbolite _formula from {old} to {new}".format(
                    old=self._formula, new=new_formula
                )
            )
    ...
    def set_inchi_string(self, new_inchi_string: str, source: str) -> int:
        ...
        if self._inchi_string is not None:
            warnings.warn(
                "changed metbolite _inchi_string from {old} to {new}".format(
                    old=self._inchi_string, new=new_inchi_string
                )
            )
    ...
    def add_inchi_string(self, new_inchi_string: str, source: str) -> int:
        ...
        if self._inchi_string != None:
            ...
            new = findOptimalInchi(
                [self._inchi_string, new_inchi_string], charge=self._charge
            )
            if new is None:
    ...
    def set_charge(self, new_charge: int) -> None:
        ...
        if self._charge is not None:
            warnings.warn(
                "changed metbolite _charge from {old} to {new}".format(
                    old=str(self._charge), new=str(new_charge)
                )
            )
    ...
    def set_pKa(self, new_pKa: dict[str, float]) -> None:
        ...
        if self._pKa is not None and self._pKa != {}:
            ...
            warnings.warn(
                "changed metabolite _pKs from {old} to {new}".format(old=old, new=new)
            )
        self._pKa = new_pKa
    ...
    def set_annotations(
        self,
        new_annotations: dict[str, list[str]],
        source: str | dict[str, list[str]] | None = None,
    ) -> None:
        ...
        if new_annotations.keys() != source.keys():
            raise ValueError(
                """Keys of the new annotation dictionary differs from the keys in source. 
            Annotation keys: {key1}
            Source keys: {key2}""".format(
                    key1=new_annotations.keys(), key2=source.keys()
                )
            )
        elif [len(x) for x in new_annotations.values()] != [
            len(y) for y in source.values()
        ]:
            raise ValueError(
                """Entries length differs for new annotation and source dictionary. Consider to provide a source dictionary via the source value"""
            )
        ...
        if warntrigger == True:
            warnings.warn(
                "changed metbolite annotations from {old} to {new}".format(
                    new=str(self.annotations), old=str(old_annotations)
                )
            )

    def add_annotations(
        self, new_annotations: dict[str, list[str]], source: str
    ) -> int:
        ...
        for x in new_annotations.keys():
            ...
            if x in self.annotations.keys():
                ...
                # remove duplicates and sort list
                new_annolst = list(
                    dict.fromkeys(new_annotations[x])
                )  # hacky way to create uniques
                # here we extend the list number of source by the number of new annotations
    ...
    def merge(
        ...
        if (
            all(
                [
                    x == y
                    for x, y in zip(
                        self.get_unique_attributes(),
                        new_metabolite.get_unique_attributes(),
                    )
                ]
            )
            != True
            ...
            if force == False:
                raise ValueError(
                    "Unique attributes differ in the two metabolites {old} vs. {new}".format(
                        old=str(self.get_unique_attributes()),
                        new=str(new_metabolite.get_unique_attributes()),
                    )
                )
            else:
                warnings.warn(
        ...
        if not keep_entries:
            ...
            self.set_inchi_string(
                new_metabolite._inchi_string, new_metabolite._inchi_source
            )
