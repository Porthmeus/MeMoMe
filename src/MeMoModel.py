...
from src.parseMetaboliteInfos import (
    parseMetaboliteInfoFromSBML,
    parseMetaboliteInfoFromSBMLMod,
...
class MeMoModel:
    ...
    def match(
        ...
        res_db = self.matchOnDB(model2, threshold=threshold_DB, output_dbs=output_dbs)
        res_name = self.matchOnName(
            model2, threshold=threshold_name, output_names=output_names
        )

        res = res_inchi.merge(
            res_db, how="outer", on=["met_id1", "met_id2"], suffixes=["_inchi", "_db"]
        )
        res = res.merge(
            res_name, how="outer", on=["met_id1", "met_id2"], suffixes=["", "_name"]
        )
        res = res.rename(
            columns={
                "charge_diff": "charge_diff_name",
                "inchi_string": "inchi_string_name",
            }
        )
        ...
        res["total_score"] = (
            res["inchi_score"] * 3 + res["DB_score"] * 2 + res["Name_score"]
        ) / 6
        ###############################################################################
        ...
        if keepAllMatches == False:
            ...
            keep_index = np.where(
                [
                    all(
                        res.iloc[i]["total_score"]
                        == met1_max.loc[res.iloc[i]["met_id1"]]
                    )
                    or all(
                        res.iloc[i]["total_score"]
                        == met2_max.loc[res.iloc[i]["met_id2"]]
                    )
        ...
        if keepUnmatched == True:
            miss_mets1 = list(
                set([x.id for x in self.metabolites]) - set(res["met_id1"])
            )
            missing_df1 = pd.DataFrame({"met_id1": miss_mets1})
            miss_mets2 = list(
                set([x.id for x in model2.metabolites]) - set(res["met_id2"])
            )
    ...
    def matchOnInchi(self, model2: MeMoModel) -> pd.DataFrame:
        ...
        mod1_inchis = pd.DataFrame(
            {
                "met_id": [x.id for x in [y for y in self.metabolites]],
                "inchis": [x._inchi_string for x in [y for y in self.metabolites]],
            }
        )
        mod2_inchis = pd.DataFrame(
            {
                "met_id": [x.id for x in [y for y in model2.metabolites]],
        ...
        if sum(mask1) > 0:
            mod1_inchis.loc[mask1, "charge"] = mod1_inchis.loc[mask1, "Mol"].apply(
                Chem.GetFormalCharge
            )
            mod1_inchis.loc[mask1, "neutralized_charge_inchi"] = mod1_inchis.loc[
                mask1, "Mol"
            ].apply(NeutraliseCharges2Inchi)
        else:
        ...
        if sum(mask2) > 0:
            mod2_inchis.loc[mask2, "charge"] = mod2_inchis.loc[mask2, "Mol"].apply(
                Chem.GetFormalCharge
            )
            mod2_inchis.loc[mask2, "neutralized_charge_inchi"] = mod2_inchis.loc[
                mask2, "Mol"
            ].apply(NeutraliseCharges2Inchi)
        else:
    ...
    def matchOnDB(
        self, model2: MeMoModel, threshold=0, output_dbs: bool = False
    ) -> pd.DataFrame:
    ...
    def matchOnName(
        self, model2: MeMoModel, threshold=0.6, output_names: bool = False
    ) -> pd.DataFrame:
        # compare two models by the entries in the databases
        mets1 = self.metabolites
        mets2 = model2.metabolites
