...
EntryAnnotationFunction = Callable[
    [AnnotationKey, pd.DataFrame, bool], tuple[dict, list, str]
]
...
def load_database(
    ...
    conversion_method: Callable[[str], pd.DataFrame] = lambda x: pd.DataFrame(),
) -> pd.DataFrame:
    """
...
def handleIDs(
    ...
    db_key: str,
    entry_annotation_function: EntryAnnotationFunction,
    allow_missing_dbs: bool = False,
) -> AnnotationResult:
    """
    ...
    for met in metabolites:
        if any(db[db_key] == met._id):
            ...
            new_met_anno_entry, new_names_entry, source = entry_annotation_function(
                met._id, db, allow_missing_dbs
            )
...
def handleMetabolites(
    db: pd.DataFrame,
    metabolites: List[MeMoMetabolite],
    db_key: str,
    annotation_function: EntryAnnotationFunction,
    allow_missing_dbs: bool = False,
) -> AnnotationResult:
    new_annos_added = 0
    new_names_added = 0
    ...
    for met in metabolites:
        ...
        if db_key in met.annotations.keys():
            for entry in met.annotations[db_key]:
                new_met_anno_entry, new_names_entry, source = annotation_function(
                    entry, db, allow_missing_dbs
                )
