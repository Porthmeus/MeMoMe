...
def annotateChEBI(
    ...
    chebi_db = load_database(
        get_config()["databases"]["ChEBI"]["file"],
        allow_missing_dbs,
    ...
    ids = [
        x
        for x, y in enumerate(metabolites)
        if y._inchi_string == None and "chebi" in y.annotations.keys()
    ]
    not_annotated_metabolites = len(ids)
    annotated_by_chebi = 0
    # check if chebis ids are actually present in the annotation slot
    annos = any(
        ["chebi" in x.annotations.keys() for i, x in enumerate(metabolites) if i in ids]
    )
    if annos:
        ...
        for i in ids:
            ...
            chebis = [
                int(x.replace("CHEBI:", ""))
                for x in metabolites[i].annotations["chebi"]
            ]
            ...
            annotated_by_chebi = annotated_by_chebi + metabolites[i].set_inchi_string(
                inchi, source="chebi"
            )
