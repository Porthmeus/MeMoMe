...
def annotateDB(
    ...
    """
    db = load_database(
        get_config()["databases"][db_name]["file"], allow_missing_dbs, loading_function
    )

    if db.empty:
        return AnnotationResult(0, 0, 0)
    return handleMetabolites(
        db, metabolites, annotation_key_name, entry_annotation_function
    )


def annotateDB_entry(
    entry: AnnotationKey,
    db_name: DBName,
    loading_function,
    ...
    if len(database) > 0:
        ...
        db = load_database(
            get_config()["databases"][db_name]["file"],
            allow_missing_dbs,
            loading_function,
        )
...
def annotateDB_id(
    ...
    db_name: DBName,
    db_key: DBKey,
    loading_function: Callable[[str], pd.DataFrame],
    entry_annotation_function: EntryAnnotationFunction,
    allow_missing_dbs: bool = False,
) -> AnnotationResult:
    """
