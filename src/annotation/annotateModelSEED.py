...
identifiers_file = os.path.join(
    get_database_path(), config["databases"]["Identifiers"]["file"]
)
...
def annotateModelSEED_entry(
    entry: str, database: pd.DataFrame = pd.DataFrame(), allow_missing_dbs: bool = False
) -> tuple[dict, list, str]:
    """
    ...
    if len(database) == 0:
        mseed = load_database(
            get_config()["databases"]["ModelSeed"]["file"],
            allow_missing_dbs,
...
def annotateModelSEED(
    ...
    return handleMetabolites(
        mseed, metabolites, "seed.compound", annotateModelSEED_entry
    )
