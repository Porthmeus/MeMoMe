...
def __normalize_hmdb_id(hmdb_id: AnnotationKey) -> AnnotationKey:
    """
    params: A hmdb id
    ...
    if prefix.upper() != "HMDB":
        warnings.warn(
            f"The id ({hmdb_id}) is not a HMDB id. We will not look for matches. We are taking the first for characters ({prefix}) and capitalize it ({prefix.upper()}). If those are not HMDB we consider this an error."
        )
...
def __handle_HMDB_entries(
    hmdb: pd.DataFrame, entry: AnnotationKey
) -> tuple[dict[str, list[str]], list[str], str]:
    # VMH models contain keys like HMDB00972, but in HMDB the id is HMDB0000972. I.e. the digit part is 7 long not 5.
    entry = __normalize_hmdb_id(entry)
...
def annotateHMDB_id(
    entry: AnnotationKey, allow_missing_dbs: bool = False
) -> AnnotationResult:
    raise NotImplementedError("Not sure if moddels with hmdb as id exist")
