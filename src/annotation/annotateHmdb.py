...
def __handle_HMDB_entries(hmdb: pd.DataFrame, entry: AnnotationKey) -> tuple[dict[str, list[str]], list[str], str]:
  # VMH models contain keys like HMDB00972, but in HMDB the id is HMDB0000972. I.e. the digit part is 7 long not 5.
...
def annotateHMDB_entry(entry: AnnotationKey,  database: pd.DataFrame = pd.DataFrame(), allow_missing_dbs: bool = False) -> tuple[dict, list, str]:
  return annotateDB_entry(entry, DBName("HMDB"), __load_csv, __handle_HMDB_entries, database, allow_missing_dbs)
...
def annotateHMDB_id(entry: AnnotationKey, allow_missing_dbs: bool = False) -> AnnotationResult:
