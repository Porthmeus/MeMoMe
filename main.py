...
def main(args: argparse.Namespace):
    if args.download:
        ...
        if args.output is None:
            print(
                "Please provide at path and output file name <path>/<outname>.csv",
                file=sys.stderr,
            )
        ...
        matched_model = model1.match(
            model2,
            keep1ToMany=args.keep_one_to_many,
            output_names=args.output_names,
            output_dbs=args.output_dbs,
            keepUnmatched=args.keep_unmatched,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If two metabolites got matched on a name basis, output the names that lead to this match",
    )
    parser.add_argument(
        "--output-dbs",
        action="store_true",
        default=False,
        help="If two metabolites got matched on a database basis, output the databases that lead to this match",
    )
    parser.add_argument(
        "--keep-one-to-many",
        action="store_true",
        default=False,
        help="Keep one-to-many merges",
    )
    parser.add_argument(
        "--keep-unmatched",
        action="store_true",
        default=False,
        help="Stored unmatched metabolties in the output",
    )
    parser.add_argument(
        "--download", action="store_true", help="Download all required databases"
    )
    parser.add_argument(
        "--model1", action="store", help="Path to the first model that should be merged"
    )
    parser.add_argument(
        "--model2",
        action="store",
        help="Path to the second model that should be merged",
    )
    parser.add_argument(
        "--output",
        action="store",
        help="Path where the output should be stored (as a csv)",
    )
    parser.add_argument(
        "--allow_missing_dbs",
        action="store_true",
        help="If set to true program does not abort if a databse is missing",
    )
