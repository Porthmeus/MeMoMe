...
def _load_subsystems(output_dir: Path) -> pd.Series:
    subsystems_path = output_dir.parent / "host_reaction_subsystems.csv"
    if not subsystems_path.exists():
        return pd.Series(dtype=str)
    df = pd.read_csv(subsystems_path)
    if "reaction_id" not in df.columns or "subsystem" not in df.columns:
        return pd.Series(dtype=str)
    mapping = {}
    for _, row in df.iterrows():
        rid = canonical_reaction_id(str(row["reaction_id"]))
        subsystem = row["subsystem"]
        if pd.isna(subsystem):
            subsystem = "NA"
        mapping[rid] = str(subsystem)
    return pd.Series(mapping, name="subsystem")
...
def main() -> None:
    ...
    auto_overlap = auto_rel.loc[overlap]
    manual_overlap = manual_rel.loc[overlap]
    full_df = pd.DataFrame(
        {
            "auto_rel_change": auto_overlap,
            "manual_rel_change": manual_overlap,
        }
    )

    filtered = [rid for rid in overlap if rid not in loop_set]
    scatter_df = full_df.loc[filtered]
    ...
    subsystems = _load_subsystems(args.output_dir)
    dependency_df = full_df.copy()
    dependency_df["abs_delta_rel_change"] = (dependency_df["manual_rel_change"] - dependency_df["auto_rel_change"]).abs()
    dependency_df["name"] = dependency_df.index.map(names).fillna("")
    if not subsystems.empty:
        dependency_df["subsystem"] = dependency_df.index.map(subsystems).fillna("NA")
    else:
        dependency_df["subsystem"] = "NA"
    dependency_df["part_of_loop"] = dependency_df.index.isin(loop_set)
    dependency_df = dependency_df[
        [
            "auto_rel_change",
            "manual_rel_change",
            "abs_delta_rel_change",
            "name",
            "subsystem",
            "part_of_loop",
        ]
    ]
    dependency_df.to_csv(loop_dir / "all_dependencies_with_subsystems.csv")
