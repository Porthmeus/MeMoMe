...
class Test_annotateBulkRoutines(unittest.TestCase):
    ...
    def test_annotateChEBI(self):
        ...
        self.assertTrue(
            all(
                [y == z for y, z in zip([x._inchi_string for x in metabolites], inchis)]
            )
        )
...
class Test_MiscStuff(unittest.TestCase):
    ...
    def test_1toManyMatchingOnInchi(self):
        ...
        self.assertTrue(
            all(res == res2)
        )  # it does not make sense to have differences in the 1toMany cases for the inchis
    ...
    def test_annotationCount(self):
        ...
        print(
            f"Amount of unannotated inchis {sum([x._inchi_string == None for x in mod.metabolites])}"
        )
        print(f"Annoatted {mod.annotate()}")
        print(
            f"Amount of unannotated inchis after Annotation {sum([x._inchi_string == None for x in mod.metabolites])}"
        )
...
class Test_removeDuplicates(unittest.TestCase):
    ...
    def test_detectDuplicates(self):
        ...
        mod_dup = cb.io.read_sbml_model(
            self.dat.joinpath("tiny_ecoli_keep_inchi_withduplicates.xml")
        )
