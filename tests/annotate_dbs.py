...
from src.annotation.annotateModelSEED import (
    annotateModelSEED,
    annotateModelSEED_id,
    annotateModelSEED_entry,
)
...
from src.annotation.annotateHmdb import (
    annotateHMDB_entry,
    annotateHMDB,
    annotateHMDB_id,
)
...
class Test_annotateMissingDbs(unittest.TestCase):
    ...
    def testBiggEntry(self, mock_err):
        ...
        self.assertEqual(
            annotateBiGG_entry(
                AnnotationKey(""), pd.DataFrame(), allow_missing_dbs=True
            ),
            ({}, [], "bigg"),
        )
        ...
        with self.assertRaises(FileNotFoundError):
            annotateBiGG_entry(
                AnnotationKey(""), pd.DataFrame(), allow_missing_dbs=False
            )
    ...
    def testBiggIDs(self, mock_err):
        self.makeDbInvis("BiGG.tsv")
        self.assertEqual(
            annotateBiGG_id([], allow_missing_dbs=True), AnnotationResult(0, 0, 0)
        )
        output = mock_err.getvalue().strip()
        self.assertIn("No such file or directory", output)
    ...
    def testBigg(self, mock_err):
        self.makeDbInvis("BiGG.tsv")
        self.assertEqual(
            annotateBiGG([], allow_missing_dbs=True), AnnotationResult(0, 0, 0)
        )
        output = mock_err.getvalue().strip()
        self.assertIn("No such file or directory", output)
    ...
    def testChEBI(self, mock_err):
        ...
        self.assertEqual(
            annotateChEBI([], allow_missing_dbs=True), AnnotationResult(0, 0, 0)
        )
        output = mock_err.getvalue().strip()
        self.assertIn("No such file or directory", output)
    ...
    def testSEED(self, mock_err):
        ...
        self.assertEqual(
            annotateModelSEED([], allow_missing_dbs=True), AnnotationResult(0, 0, 0)
        )
        output = mock_err.getvalue().strip()
        self.assertIn("No such file or directory", output)
    ...
    def testSEED_id(self, mock_err):
        self.makeDbInvis("modelSeed.tsv")
        self.assertEqual(
            annotateModelSEED_id([], allow_missing_dbs=True), AnnotationResult(0, 0, 0)
        )
        output = mock_err.getvalue().strip()
        self.assertIn("No such file or directory", output)
    ...
    def testSEED_entry(self, mock_err):
        self.makeDbInvis("modelSeed.tsv")
        self.assertEqual(
            annotateModelSEED_entry("", pd.DataFrame(), allow_missing_dbs=True),
            ({}, [], "Seed"),
        )
        output = mock_err.getvalue().strip()
        self.assertIn("No such file or directory", output)
    ...
    def testVMH(self, mock_err):
        self.makeDbInvis("vmh.json")
        self.assertEqual(
            annotateVMH([], allow_missing_dbs=True), AnnotationResult(0, 0, 0)
        )
        output = mock_err.getvalue().strip()
        self.assertIn("No such file or directory", output)
    ...
    def testVMH_id(self, mock_err):
        self.makeDbInvis("vmh.json")
        self.assertEqual(
            annotateVMH_id([], allow_missing_dbs=True), AnnotationResult(0, 0, 0)
        )
        output = mock_err.getvalue().strip()
        self.assertIn("No such file or directory", output)
    ...
    # Test that hmdb entry errors if the database is not found
    @patch("sys.stderr", new_callable=StringIO)
    def testHmdbEntry(self, mock_err):
        self.makeDbInvis("hmdb.csv")
        self.assertEqual(
            annotateHMDB_entry(
                AnnotationKey(""), pd.DataFrame(), allow_missing_dbs=True
            ),
        ...
        self.makeDbVis("hmdb.csvxxx")

    # We currently do not implement the id function for hmdb, if that is ever changed, we can remove this
    # NOTE if this happends we need to add HMDB to the list of origin dbs
    @patch("sys.stderr", new_callable=StringIO)
    def testHmdbIDs(self, mock_err):
        try:
            annotateHMDB_id([], allow_missing_dbs=True), AnnotationResult(0, 0, 0)
    ...
    @patch("sys.stderr", new_callable=StringIO)
    def testHmdb(self, mock_err):
        self.makeDbInvis("hmdb.csv")
        self.assertEqual(
            annotateHMDB([], allow_missing_dbs=True), AnnotationResult(0, 0, 0)
        )
        output = mock_err.getvalue().strip()
        self.assertIn("No such file or directory", output)

        with self.assertRaises(FileNotFoundError):
            annotateHMDB([], allow_missing_dbs=False)
            self.assertIn("No such file or directory", output)
        self.makeDbVis("hmdb.csvxxx")
...
class Test_annotateEntryFunctions(unittest.TestCase):
    ...
    def testBiggEntry(self):
        ret = annotateBiGG_entry(AnnotationKey("13dpg"), allow_missing_dbs=False)
        self.assertTrue(len(ret[0]) > 0)
        self.assertTrue(len(ret[1]) > 0)

        ret = annotateBiGG_entry(AnnotationKey(""), allow_missing_dbs=False)
        self.assertEqual(ret, (dict(), list(), "bigg"))

    def testHmdbEntry(self):
        ret = annotateHMDB_entry(AnnotationKey("HMDB0000001"), allow_missing_dbs=False)
        self.assertTrue(len(ret[0]) > 0)
        self.assertTrue(len(ret[1]) > 0)
        self.assertEqual(ret[2], "HMDB")

        ret = annotateHMDB_entry(AnnotationKey(""), allow_missing_dbs=False)
        self.assertEqual(ret, (dict(), list(), "HMDB"))
    ...
    def testBiggEntryToy(self):
        ...
        data = {
            ...
            "database_links": [
                ...
                "http://identifiers.org/A/A_M2",
                "http://identifiers.org/A/A_M3",
            ],
        }
    ...
    def testHmdbEntryToy(self):
        ...
        # Create mock data for 3 entries
        data = [
            [
                "HMDB0000001",
                "H2O",
        ...
        ret = annotateBiGG_entry("HMDB0000001", df, allow_missing_dbs=False)
    ...
    def testBiggHandler(self):
        ...
        self.assertTrue(
            set(["HMDB02322", "HMDB04567", "HMDB07890"]) == set(res["hmdb"])
        )
...
class Test_annotateID(unittest.TestCase):
    ...
    def testVMH_id(self):
        ...
        expected_annotations = {
            ...
            "inchiKey": ["AUFGTPPARQZWDO-YUZLPWPTSA-L"],
            "smile": [
                "[H]OC1=NC(=NC2=C1N([H])C([H])(C([H])([H])N(C([H])=O)C1=C([H])C([H])=C(C([H])=C1[H])C(=O)N([H])[C@]([H])(C([O-])=O)C([H])([H])C([H])([H])C([O-])=O)C([H])([H])N2[H])N([H])[H]"
            ],
            "hmdb": ["HMDB0000972"],
            "metanetx": ["MNXM237"],
            "seed": ["cpd00201"],
