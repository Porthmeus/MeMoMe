...
class Test_removeDuplicateMetabolites(unittest.TestCase):
    ...
    def createToyModel(self):
        ...
        reactionAB.lower_bound = -30.  
        reactionAB.upper_bound = 40.  

        reactionAD = cb.Reaction('R_AD')
        reactionAD.name = 'A+D=C'
        reactionAD.subsystem = 'test'
        reactionAD.lower_bound = 0.  # This is the default
        reactionAD.upper_bound = 1000.  # This is the default

        reactionDE = cb.Reaction("R_DE")
        reactionDE.name = "D+E = F"
        reactionDE.subsystem = 'test'
        reactionDE.lower_bound = 0.  # This is the default
        reactionDE.upper_bound = 1000.  # This is the default
        ...
        reactionAB.add_metabolites({
            A: -1.0,
            B: -1.0,
            C: 1.0
        })
        reactionAD.add_metabolites({
            A: -1.0,
            D: -1.0,
            C: 1.0
        })
        reactionDE.add_metabolites({
            E: -1.0,
            D: -1.0,
            F: 1.0
        })
    ...
    def test_exchangeMetabolite(self):
        ...
        #print(res)
        self.assertTrue(all([x == y for x,y in zip(rxns_forms, [z.reaction for z in toy.reactions])])== False)
